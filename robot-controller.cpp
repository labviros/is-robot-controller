...
is::pb::Duration get_period(SyncRequest const& sync_request) {
  double period;
  if (sync_request.sampling().rate_case() == SamplingSettings::RateCase::kFrequency) {
    period = 1.0 / sync_request.sampling().frequency();
  } else {
    period = sync_request.sampling().period();
  }
  return is::pb::TimeUtil::NanosecondsToDuration(static_cast<int64_t>(1e9 * period));
}
...
int main(int argc, char* argv[]) {
  ...
  is::pb::Duration period = get_period(sync_request);
  ...
  provider.delegate<RobotTask, is::pb::Empty>(
      provider_tag, "SetTask", [&](RobotTask const& robot_task, is::pb::Empty*) -> Status {
        ...
        *sync_request.mutable_sampling() = robot_task.sampling();
        period = get_period(sync_request);
        is::info("{}", sync_request);
  ...
  while (1) {
    ...
    auto sampling_deadline = window_end - 0.2 * period;
    ...
    switch (state) {
      ...
      case COMPUTE_COMMAND: {
        ...
        auto inject_span = last_annotation != annotations.end();
        std::unique_ptr<is::ot::Span> span;
        if (inject_span)
          span = tracer.extract(*last_annotation, "control");
        ...
        *controller_status.mutable_current_speed() = fence::limit_speed(controller_status, parameters.fence());
        controller_status.set_sources(annotations.size());
        ...
        *robot_config.mutable_speed() = speed;
        ...
        speed_msg->Expiration("50"); // milliseconds
        if (inject_span)
        ...
        if ((!is_sync(timestamps, period) || do_sync ) && !waiting_sync) {
